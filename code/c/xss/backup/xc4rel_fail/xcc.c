...
int *code,    // ccc: add *code (程式段)
    *e, *le,  // current position in emitted code (e: 目前機器碼指標, le: 上一行機器碼指標)
    ...
    output=0; // output target file. (輸出目的檔) (add by ccc)
...
void expr(int lev) // 運算式 expression, 其中 lev 代表優先等級
{
  ...
  while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method
    ...
    if (tk == Assign) {
      ...
      next();
      *++e = BZ; d = ++e; // 不成立時 BZ 到 else 開始那個點 *d = (int)(e + 3 - d);。
      expr(Assign);
      if (tk == ':') next(); else { printf("%d: conditional missing colon\n", line); exit(-1); }
      *d = (int)(e + 3 - d); *++e = JMP; d = ++e; // JMP *d;   *d = Cond 結束後的那個程式點。 (ccc: 目前用絕對跳躍，是否應該改相對跳曜)
      expr(Cond);
      *d = (int)(e + 1 - d);
    }
...
int main(int argc, char **argv) // 主程式
{
  ...
  if (argc > 0 && **argv == '-' && (*argv)[1] == 'o') { output = 1; --argc; ++argv; } // output: add by ccc
